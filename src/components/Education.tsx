...
const Education: React.FC = () => {
  return (
    <section id="education" className="education min-h-screen py-20 px-6 bg-dark/50">
...
const EducationCard: React.FC<{ item: EducationItem; index: number }> = ({ item, index }) => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className={`w-full md:w-5/12 ${item.position === 'left' ? 'md:text-right' : 'md:ml-auto'}`}>
        <div className={`bg-dark/80 p-6 rounded-lg shadow-xl transition-all duration-1000 ${
          isIntersecting 
            ? `opacity-100 ${item.position === 'left' ? 'translate-x-0' : 'translate-x-0'}`
            : `opacity-0 ${item.position === 'left' ? '-translate-x-10' : 'translate-x-10'}`
        }`}>
